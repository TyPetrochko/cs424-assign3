...
int resize_matrix(double *from, double *to, int from_size, int to_size){
  int idx, i, j;

  for(i = 0, idx = 0; i < to_size; i++)
    for(j = 0; j < to_size; j++, idx++)
      to[idx] = from[(from_size * i) + j];
}
...
void matrix_init(int, int, double**, double**, double**);
...
int main(int argc, char **argv) {
  ...
  */
  ...
  int N, i, j, k, run, rows, N_orig;
  double *A, *B, *C, *C_resized, *C2; /* master buffers */
  ...
  if (rank == ROOT) { /* master */
    ...
    for (run=0; run<NUM_RUNS; run++) {
      ...
      N_orig = sizes[run];
      N = pad(N_orig, size); /* pad it! */
      
      matrix_init(N_orig, size, &A, &B, &C);
      C_resized = (double *) calloc(N_orig * N_orig, sizeof(double));

      buffer_init(N / size, N, &Ablock, &Bblock, &B2block, &Cblock);
      timing(&wctime0, &cputime);
      master(size, N, A, B, C, Ablock, Bblock, B2block, Cblock);
      resize_matrix(C, C_resized, N, N_orig);
      N = N_orig; // abandon padding
      ...
      if(VERIFY){
        ...
        matmul(N, A, B, C2);
        for(i = 0; i < N * N; i++)
          if((float)C_resized[i] != (float)C2[i])
            printf("ERROR: element number %d is %f but should be %f\n", i, C_resized[i], C2[i]);
        printf("Done verifying correctness, any errors have been reported\n");
        free(C2);
      }
      free(A); free(Ablock);
      free(B); free(Bblock);
      free(C); free(Cblock); free(C_resized);
      free(B2block);
    }
  } else { /* worker */
    for (run=0; run<NUM_RUNS; run++) {
...
void matrix_init(int N, int p, double **A, double **B, double **C){
  int sizeAB, sizeC, sizeAB_padded, sizeC_padded, i, j, N_padded;
  ...
  sizeAB_padded = N_padded * (N_padded + 1) / 2;
  sizeC_padded = N_padded * N_padded;

  *A = (double *) calloc(sizeAB_padded, sizeof(double));
  *B = (double *) calloc(sizeAB_padded, sizeof(double));
  *C = (double *) calloc(sizeC_padded, sizeof(double));
